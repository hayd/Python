...
class Printer():
    ...
    def flush(self):
        send(currentClient, "editor.eval.python.print",
             {"msg": self.cur, "file": name})
        self.cur = ""
    ...
    for form in toExec:
        ...
        try:
            ...
            try:
                ...
                e = traceback.format_exc()
                send(data[0], "editor.eval.python.exception",
                     {"ex": cleanTrace(e), "meta": loc})
                continue

        try:
            if isEval:
                result = eval(code, module.__dict__)
                send(data[0], "editor.eval.python.result",
                     {"meta": loc, "result": asUnicode(result)})
            else:
...
def ipyEval(data):
    ...
    for form in toExec:
        ...
        ltipy.request({"meta": loc, "name": data[2]["name"], "path": path, "code": code, "client": data[
                      0], "evaltype": "expression" if isEval else "statement"})
...
if __name__ == "__main__":
    ...
    info = {'name': name, 'client-id': cid,
            'dir': curDir, 'commands': ['editor.eval.python']}
